...
import com.shodh.lms.NotificationActivity;
...
public class NotificationLiveViewModel extends ViewModel {

    private MutableLiveData<JSONArray> notification;

    public NotificationLiveViewModel() {
    ...
    public void makeApiCall(Context context,SharedPreferences user){
        ...
        StringRequest cacheRequest = new StringRequest(
                Request.Method.GET,
                Constants.GET_ALL_NOTIFICATION,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.i("LMS_TEST", "onResponse: "+response);
                        try {
                            JSONArray jsonArray = new JSONObject(response).getJSONArray("all_notifications");
